...
def run_xtb(input_filename, method, charge, optimize, solvent, solvent_name):
    try:
...
#charge = st.number_input("Enter charge of the molecule:", value=0, step=1, format="%d", min_value=-10, max_value=10)

optimize = st.checkbox("Optimize geometry?", value=True)

method = st.radio("Select method for xtb:", ['gfn0', 'gfn1', 'gfn2', 'gfnff'], index=2)

# Charge input (small box)
#charge = st.number_input("Enter charge of the molecule:", value=0, step=1, format="%d", min_value=-10, max_value=10)
...
if st.button("Run"):
    if smiles:
        ...
        if smiles_to_xyz(smiles, 'input_molecule.xyz'):
            ...
            if run_xtb('input_molecule.xyz', method, charge, optimize, solvent, solvent_name):
